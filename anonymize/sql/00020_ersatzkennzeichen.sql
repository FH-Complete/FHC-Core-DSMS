------------------------------------------------------------------------------------------------------------------------
	-- ANONYMISIERUNG_SQL
------------------------------------------------------------------------------------------------------------------------
UPDATE public.tbl_person set ersatzkennzeichen=null WHERE person_id in(SELECT
		person_id
	FROM
		public.tbl_person
	WHERE
		...
		AND ersatzkennzeichen is not null
	)
...
SELECT
	count(*) as anzahl
	--person_id, vorname, nachname, ersatzkennzeichen
FROM
	public.tbl_person
WHERE
	-- kein Mitarbeiter
